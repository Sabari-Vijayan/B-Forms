...
import { useGetForm, usePublishForm } from "@/hooks/api";
...
import { Loader2, Copy, ExternalLink, QrCode, CheckCircle2, AlertCircle, Download, ArrowLeft } from "lucide-react";
import { QRCodeSVG } from "qrcode.react";
import { SUPPORTED_LANGUAGES } from "@/lib/constants";
import Link from "next/link";

export default function FormSharePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          <Link href={`/forms/${id}`}>
            <Button variant="ghost" size="sm" className="-ml-2 h-8 px-2 text-muted-foreground hover:text-foreground">
